...
export const metadata: Metadata = {
  title: "Contact",
  description:
    "Contact PCI AVSS in Anaheim, CA for security cameras, alarms, access control, video monitoring, and mobile surveillance solutions.",
...
export default function ContactPage() {
  return (
    <main className="min-h-screen bg-brand-ink-950 text-white">
        ...
        <div className="container mx-auto w-full">
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
            {contactOptions.map((item) => (
              ...
              >
                <span className="inline-flex rounded-full bg-white/5 p-3 text-brand-steel-300">
                  {item.icon}
                </span>
        ...
        <div className="container mx-auto w-full">
          <div className="grid gap-6 lg:grid-cols-12">
            <div className="rounded-[24px] border border-white/10 bg-gradient-to-br from-white/10 to-white/5 p-6 shadow-[0_18px_44px_rgba(0,0,0,0.35)] lg:col-span-7 lg:p-8">
              ...
              <div className="mt-7 grid gap-3">
                {visitReasons.map((reason) => (
                  ...
                  >
                    <span className="mt-0.5 rounded-full bg-white/5 p-2 text-brand-steel-300">
                      <SendIcon />
                    </span>
            ...
            <div className="space-y-6 lg:col-span-5">
              <div className="rounded-[24px] border border-white/10 bg-brand-ink-950 p-6 shadow-[0_18px_44px_rgba(0,0,0,0.28)]">
                <div className="flex items-start gap-3">
                  <span className="rounded-full bg-white/5 p-3 text-brand-steel-300">
                    <MapPinIcon />
                  </span>
                  <div>
                    <p className="text-sm font-semibold uppercase tracking-[0.18em] text-brand-steel-300">
                    ...
                    <a
                      ...
                      className="mt-5 inline-flex text-sm font-black uppercase tracking-[0.14em] text-brand-gold-500 transition hover:text-brand-gold-600"
                    >
                      Get Directions
                    </a>
                  </div>
                  ...
                  <div className="w-full">
                    ...
                    <div className="mt-4 space-y-3">
                      {siteContact.businessHours.map((item) => (
                        <div
                          key={item}
                          className="flex flex-col gap-1 border-b border-white/10 pb-3 text-sm text-white/75 last:border-b-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between sm:gap-4"
                        >
                          <span>{item.split(" - ")[0]}</span>
                          <span className="font-semibold uppercase tracking-widest text-white/55">
                            {item.split(" - ")[1]}
                          </span>
                        </div>
                      ))}
      ...
      <section className="bg-brand-ink-800 px-4 pb-10 sm:px-8 sm:pb-14">
        <div className="container mx-auto w-full">
